...
export class TestScheduler extends VirtualTimeScheduler {
  ...
  constructor(public assertDeepEqual: (actual: any, expected: any) => boolean | void) {
    super(VirtualAction as any, defaultMaxFrame);
  }
        ...
        default:
          ...
          if (runMode && c.match(/^[0-9]$/)) {
            ...
            if (i === 0 || marbles[i - 1] === ' ') {
              ...
              if (match) {
                i += match[0].length - 1;
                const duration = parseFloat(match[firstMatch]);
                const unit = match[secondMatch];
                let durationInMs = 0;
                ...
                switch (unit) {
                  case 'ms':
                    ...
                    break;
                  case 's':
                    durationInMs = duration * msInSec;
                    break;
                  case 'm':
                    durationInMs = duration * msInSec * secInMin;
                    break;
                  default:
                    break;
                }
  ...
  expectObservable(
    ...
    const unsubscriptionFrame = TestScheduler.parseMarblesAsSubscriptions(unsubscriptionMarbles, this.runMode)
      .unsubscribedFrame;
  ...
  expectSubscriptions(actualSubscriptionLogs: SubscriptionLog[]): { toBe: subscriptionLogsToBeFn } {
    ...
    return {
      toBe(marbles: string | string[]) {
        ...
        flushTest.ready = true;
        flushTest.expected = marblesArray.map(m => TestScheduler.parseMarblesAsSubscriptions(m, runMode));
      },
    };
  }
  ...
  flush() {
    ...
    while (hotObservables.length > 0) {
      const observable = hotObservables.shift();
      observable && observable.setup();
    }
  ...
  run<T>(callback: (helpers: RunHelpers) => T): T {
    ...
    this.runMode = true;
    // @ts-ignore: exists in RxJs6 but not in RxJs5
